...
if (!global.sinhalacartoonsSessions) global.sinhalacartoonsSessions = {};

module.exports = async ({ sock, from, msg, args }) => {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';

    if (global.sinhalacartoonsSessions[from] && !text.startsWith('.')) {
        const session = global.sinhalacartoonsSessions[from];
        ...
        if (!isNaN(choice) && choice > 0 && choice <= session.links.length) {
            ...
            delete global.sinhalacartoonsSessions[from];
            return;
        }
    }

    if (!text.startsWith('.sinhalacartoons')) return;
    if (!args) return await sock.sendMessage(from, { text: '❌ කරුණාකර සෙවිය යුතු නම ඇතුළත් කරන්න! (eg: .sinhalacartoons Ben10)' });

    await sock.sendMessage(from, { text: `🔍 *"${args}"* සඳහා Sinhalacartoons.com පිරික්සමින් පවතී...` });

    try {
        const searchUrl = `https://sinhalacartoons.com/?s=${encodeURIComponent(args)}`;
        ...
        const cartoon = results[0];
        const pageData = await axios.get(cartoon.link, { headers: { 'User-Agent': 'Mozilla/5.0' } });
        ...
        global.sinhalacartoonsSessions[from] = { title: cartoon.title, links: links };
        let menuText = `🎬 *${cartoon.title}*\n\n📥 *SINHALACARTOONS LINKS :*\n`;
