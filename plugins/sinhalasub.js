...
if (!global.sinhalasubSessions) global.sinhalasubSessions = {};

module.exports = async ({ sock, from, msg, args }) => {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';

    if (global.sinhalasubSessions[from] && !text.startsWith('.')) {
        const session = global.sinhalasubSessions[from];
        ...
        if (!isNaN(choice) && choice > 0 && choice <= session.links.length) {
            ...
            await sock.sendMessage(from, { text: `📥 *${session.title}* චිත්‍රපටය සර්වර් එකට බාගත වෙමින් පවතී...` });
            ...
            delete global.sinhalasubSessions[from];
            return;
        }
    }

    if (!text.startsWith('.sinhalasub')) return;
    if (!args) return await sock.sendMessage(from, { text: '❌ කරුණාකර සෙවිය යුතු නම ඇතුළත් කරන්න! (eg: .sinhalasub Iron Man)' });

    await sock.sendMessage(from, { text: `🔍 *"${args}"* සඳහා SinhalaSub පිරික්සමින් පවතී...` });
    ...
    try {
        ...
        global.sinhalasubSessions[from] = { title: movie.title, links: links };
        let menuText = `🎬 *${movie.title}*\n\n📥 *SINHALASUB DOWNLOAD LINKS :*\n`;
