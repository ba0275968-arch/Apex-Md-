...
if (!global.animeclubSessions) global.animeclubSessions = {};

module.exports = async ({ sock, from, msg, args }) => {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';

    if (global.animeclubSessions[from] && !text.startsWith('.')) {
        const session = global.animeclubSessions[from];
        ...
        if (!isNaN(choice) && choice > 0 && choice <= session.links.length) {
            ...
            await sock.sendMessage(from, { text: `📥 *${session.title}* ඇනිමේ වීඩියෝව සර්වර් එකට බාගත වෙමින් පවතී...` });
            ...
            delete global.animeclubSessions[from];
            return;
        }
    }

    if (!text.startsWith('.animeclub')) return;
    if (!args) return await sock.sendMessage(from, { text: '❌ කරුණාකර සෙවිය යුතු ඇනිමේ නම ඇතුළත් කරන්න! (eg: .animeclub One Piece)' });

    await sock.sendMessage(from, { text: `🔍 *"${args}"* සඳහා AnimeClub පිරික්සමින් පවතී...` });

    try {
        const searchUrl = `https://animeclub.lk/?s=${encodeURIComponent(args)}`;
        ...
        global.animeclubSessions[from] = { title: anime.title, links: links };
