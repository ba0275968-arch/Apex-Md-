...
if (!global.zoomlkSessions) global.zoomlkSessions = {};

module.exports = async ({ sock, from, msg, args }) => {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';

    if (global.zoomlkSessions[from] && !text.startsWith('.')) {
        const session = global.zoomlkSessions[from];
        ...
        if (!isNaN(choice) && choice > 0 && choice <= session.links.length) {
            ...
            delete global.zoomlkSessions[from];
            return;
        }
    }

    if (!text.startsWith('.zoomlk')) return;
    if (!args) return await sock.sendMessage(from, { text: '❌ කරුණාකර සෙවිය යුතු නම ඇතුළත් කරන්න! (eg: .zoomlk Interstellar)' });

    await sock.sendMessage(from, { text: `🔍 *"${args}"* සඳහා ZoomLK පිරික්සමින් පවතී...` });

    try {
        const searchUrl = `https://zoom.lk/?s=${encodeURIComponent(args)}`;
        ...
        global.zoomlkSessions[from] = { title: movie.title, links: links };
        let menuText = `🎬 *${movie.title}*\n\n📥 *ZOOMLK DOWNLOAD LINKS :*\n`;
