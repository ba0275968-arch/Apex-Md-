...
if (!global.animostSessions) global.animostSessions = {};

module.exports = async ({ sock, from, msg, args }) => {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';

    if (global.animostSessions[from] && !text.startsWith('.')) {
        const session = global.animostSessions[from];
        ...
        if (!isNaN(choice) && choice > 0 && choice <= session.links.length) {
            ...
            delete global.animostSessions[from];
            return;
        }
    }

    if (!text.startsWith('.animost')) return;
    if (!args) return await sock.sendMessage(from, { text: '❌ කරුණාකර සෙවිය යුතු නම ඇතුළත් කරන්න! (eg: .animost Naruto)' });

    await sock.sendMessage(from, { text: `🔍 *"${args}"* සඳහා Animost පිරික්සමින් පවතී...` });

    try {
        const searchUrl = `https://animost.com/?s=${encodeURIComponent(args)}`;
        ...
        global.animostSessions[from] = { title: anime.title, links: links };
